...
def get_last_user_after_assistant(history):
    """反向遍历聊天历史，找到最后一个assistant位置，并返回后面的所有user消息"""
    # 检查聊天历史是否为空
    if not history:
        return None

    # 最后一条消息应该是user消息（由add_message保证），但还是要检查一下
    if history[-1]['role'] != 'user':
        return None

    # 反向遍历历史，找到最后一个assistant的位置
    ...
    # 如果没有找到assistant，返回所有user消息
    if last_assistant_index == -1:
        return [msg for msg in history if msg['role'] == 'user']

    # 获取最后一个assistant之后的所有user消息
    user_messages = []
    for i in range(last_assistant_index + 1, len(history)):
        if history[i]['role'] == 'user':
            user_messages.append(history[i])
...
def transcribe_image(image_path):
    return {
        'type': 'image_url',
        'image_url': {'url': f'data:image/{image_to_base64(image_path)[0]}'},
        'detail': 'low'
    }
...
async def submit_llm(history: List[Dict]):
    ...
    if user_input_messages:
        for msg in user_input_messages:
            if isinstance(msg['content'], str):
                # context.append({'type': 'text', 'text': msg['content']})
                context.extend(msg['content'])

            elif isinstance(msg['content'], tuple):
                file_path = msg['content'][0]
                if file_path.endwoth('.jpg') or file_path.endwoth('.png') or file_path.endwoth('.jpeg'):
                    file_message = transcribe_image(file_path)
                    context.append(file_message)
            else:
                pass
    ...
    async for chunk in graph.astream(inputs, config, stream_mode=['messages','updates']): # 同时监听消息和状态更新
        if isinstance(chunk,tuple) and 'messages' in chunk:
            for message in chunk[1]:
                ...
                if isinstance(message,AIMessage) and message.content:
                    ...
                    # 更新最后一条消息非追加
                    if history and isinstance(history[-1],ChatMessage) and 'title'not in history[-1].metadata:
                        history[-1].context = full_response
                    else:
                        history.append(ChatMessage(role='assistamt',context=message.content))
                    yield history

                elif isinstance(message, ToolMessage):
                    tool_msg = f" 调用工具: {message.name}\n{message.content}"
                    history.append(ChatMessage (role="assistant", content=tool_msg,metadata={"title": f" Used tool {message.name}"}))
                    yield history
    ...
    if current_state.next:  # 出现了工作流的中断
        ...
        history.append(ChatMessage(role='assistant',context=output))
        yield history
    else:
...
with gr.Blocks(title='多模态RAG项目', theme=gr.themes.Soft(), css=css) as instance:
    gr.Label('梧桐叶的多模态RAG项目', container=False)

    chatbot = gr.Chatbot( height=600, label='AI助手',
